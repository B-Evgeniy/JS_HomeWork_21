...
function replaceCSSComments(css) {
  let regex = /\/\*.*?\*\/\s?/gui;
...
function checkYear(year) {
  let regex = /^(19\d\d|20\d\d|2100)$/gui;
...
function checkEmail(eMail) {
  let regex = /^[a-z]+\W?[a-z]+@[a-z\.]+\.[a-z]{2,3}$/gui;
...
function checkDomainUrl(url) {
  let regex = /^(http:|https:)\/\/(?!:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}?$/gui;
...
function createLinksFromDomains(url) {
  let regex = new RegExp('(http:|https:)\\/\\/(?!:\\/\\/)([a-zA-Z0-9-_]+\\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\\.[a-zA-Z]{2,5}', 'gui');
  url.match(regex).forEach((item) => url = url.replace(item, `<a href="${item}" target="_blank">${item.replace(/^(http:|https:)\/\//gui, '')}</a>`));
  return url;
}
